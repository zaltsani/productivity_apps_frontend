...
import type { CalendarProps } from "./type/calendar"
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute left-5 top-5 border-0",
          disableLeftNavigation() && "pointer-events-none",
        ),
        button_next: cn(
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute right-5 top-5 border-0",
      ...
      components={{
        Chevron: ({ ...props }) =>
          props.orientation === "left" ? <ChevronLeft className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />,
